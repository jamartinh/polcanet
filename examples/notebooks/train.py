...
from polcanet.aencoders import ConvEncoder, LinearDecoder, LinearConvDecoder, EfficientLinearConvDecoder
...
def train(params):
    ...
    if params.use_dataloader or (X.shape[0] >= 30_000 and params.n_channels == 3) or (X.shape[0] >= 70_000):
        train_with_dataloader(X, y, model, batch_size, lr_schedule, epoch_list, params, optimizer=optimizer,
...
def create_polca_model(experiment, input_dim, latent_dim, labels, params) -> PolcaNet:
    ...
    if params.with_labels:
        r = params.r if params.r is not None else 1
        c = params.c if params.c is not None else 1e-1
...
def train_with_dataloader(X, y, model, batch_size, lr_schedule, epoch_list, params, optimizer, X_val=None, y_val=None):
    ...
    y_val_torch = torch.tensor(y_val, dtype=torch.int64) if params.with_labels else None
    test_data_set = TensorDataset(x_val_torch, y_val_torch) if params.with_labels else TensorDataset(x_val_torch)
...
def train_in_memory(X, y, model, batch_size, lr_schedule, epoch_list, optimizer, X_val=None, y_val=None):
    ...
    print("Training with in-memory data")
    for lr, epochs in zip(lr_schedule, epoch_list):
        # Update learning rate
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
